...
Ausgangehend von der Gleichung (4.47) im Skript mit $n=N$:
\begin{flalign*}
	P_k &= \frac{2}{N^2}(\Re(X[k]))^2 + (\Im(X[k]))^2  \\
	P_k &= (\Re(X[k]))^2 + (\Im(X[k]))^2 \\
	P_k &= |y_k[N]|^2 = |W_N^{-k}*y_k[N-1]+x[N]|^2 \\
	W_N^{-k}&= (e^{-\frac{j*2*\pi}{N}})^{-k} \\
	P_k &= s[N-1 \\
	P_k &= |y_k[N]|^2 = delay[1]^2 + delay[2]^2 + coefficient * delay[1]*delay[2]
\end{flalign*}
